...
조정합니다. (Agent당 LLM 호출이 1차+2차로 2회씩 발생해 API 비용이 기존 대비 약 2배입니다.)

**Validator Agent**는 각 Agent의 판단이 실제 근거 데이터에 기반하는지 확인합니다.
데이터 수집 실패 등으로 근거 데이터가 아예 없는 Agent는 (LLM에게 물어볼 것도 없이)
`grounded=False`로 확정하고, 근거가 있는 Agent에 한해서만 reason/detail이 실제로 그
데이터와 앞뒤가 맞는지 LLM으로 교차 검증합니다. `grounded=False`로 판정된 Agent는
Orchestrator의 가중 평균에서 완전히 제외됩니다 — 근거 없는 "관망 50점"이 마치 정상
의견인 것처럼 최종 판단에 섞여 들어가는 걸 막기 위함입니다. 화면에서는 제외된 Agent의
카드가 점선 테두리로 흐리게 표시되고, 최종 판단에도 어떤 Agent가 왜 제외됐는지 나옵니다.
...
본 분석은 참고용이며, 실제 투자 결정과 그에 따른 책임은 전적으로 사용자 본인에게 있습니다. 그래?
